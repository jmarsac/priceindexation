...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_crabe_priceindexation.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from crabe model with price indexation
 */
...
/**
 *	Class to manage PDF invoice template Crabe for price indexation
 */
class pdf_crabe_priceindexation extends ModelePDFFactures
{
     /**
	...
	// extrafields positions
	// float X position for the initial unit price
	var $posxinitialup;
	// float X position for the initial price index
	var $posxinitialindex;
	// float X position for the current price index
	var $posxcurrentindex;
	...
	function __construct($db)
	{
		...
		// Translations
		$langs->loadLangs(array("main", "bills", "priceindexation@priceindexation"));

		$this->db = $db;
		$this->name = "crabe priceindexation";
		$this->description = $langs->trans('PDFCrabePriceIndexationDescription');
		...
		if($conf->global->PRODUCT_USE_UNITS)
		{
			 ...
			 */
			//
			$this->posxinitialup=63;
			$this->posxinitialindex=79;
			$this->posxcurrentindex=95;
			$this->posxtva=111;
			$this->posxup=128;
			$this->posxqty=145;
			$this->posxunit=161;
		...
		else
		{
			...
			$this->posxqty=145;
			*/
			$this->posxinitialup=72;
			$this->posxinitialindex=88;
			$this->posxcurrentindex=104;
			$this->posxtva=120;
			$this->posxup=136;
			$this->posxqty=155;
		}
		...
		$this->posxprogress=136; // Only displayed for situation invoices
		$this->postotalht=180;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		// Translations
		$outputlangs->loadLangs(array("main", "bills", "products", "dict", "companies", "priceindexation@priceindexation"));
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				{
					$this->posxinitialup+=($this->postotalht - $this->posxdiscount);
					$this->posxinitialindex+=($this->postotalht - $this->posxdiscount);
					$this->posxcurrentindex+=($this->postotalht - $this->posxdiscount);
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					$optionkeyname="price0";
					$initialup = $extrafieldsline->showOutputField($optionkeyname, $object->lines[$i]->array_options['options_'.$optionkeyname]);
					$optionkeyname="index0";
					$initialindex = $extrafieldsline->showOutputField($optionkeyname, $object->lines[$i]->array_options['options_'.$optionkeyname]);
					$optionkeyname="index1";
					$currentindex = $extrafieldsline->showOutputField($optionkeyname, $object->lines[$i]->array_options['options_'.$optionkeyname]);
					...
					// Initial Unit Price before indexation
					//$initialup = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					//$initialup = $outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_price0']);
					$pdf->SetXY($this->posxinitialup, $curY);
					$pdf->MultiCell($this->posxinitialindex-$this->posxinitialup-0.8, 3, $initialup, 0, 'R', 0);

					// Initial price index
					//$initialindex = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					//$initialindex = $outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_index0']);
					$pdf->SetXY($this->posxinitialindex, $curY);
					$pdf->MultiCell($this->posxcurrentindex-$this->posxinitialindex-0.8, 3, $initialindex, 0, 'R', 0);

					// Current price index
					//$currentindex = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails);
					//$currentindex = $outputlangs->convToOutputCharset($object->lines[$i]->array_options['options_index1']);
					$pdf->SetXY($this->posxcurrentindex, $curY);
					$pdf->MultiCell($this->posxtva-$this->posxcurrentindex-0.8, 3, $currentindex, 0, 'R', 0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		dol_syslog(get_class($this) . ':: this->posxinitialup=' . $this->posxinitialup, LOG_DEBUG);
		$pdf->line($this->posxinitialup-1, $tab_top, $this->posxinitialup-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxinitialup-1, $tab_top+1);
			$pdf->MultiCell($this->posxinitialindex-$this->posxinitialup-1,2, $outputlangs->transnoentities("Initial_UP"),'','C');
		}

		dol_syslog(get_class($this) . ':: this->posxinitialindex=' . $this->posxinitialindex, LOG_DEBUG);
		$pdf->line($this->posxinitialindex-1, $tab_top, $this->posxinitialindex-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxinitialindex-1, $tab_top+1);
			$pdf->MultiCell($this->posxcurrentindex-$this->posxinitialindex-1,2, $outputlangs->transnoentities("Initial_index"),'','C');
		}

		dol_syslog(get_class($this) . ':: this->posxcurrentindex=' . $this->posxcurrentindex, LOG_DEBUG);
		$pdf->line($this->posxcurrentindex-1, $tab_top, $this->posxcurrentindex-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxcurrentindex-1, $tab_top+1);
			$pdf->MultiCell($this->posxtva-$this->posxcurrentindex-1,2, $outputlangs->transnoentities("Current_index"),'','C');
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		// Translations
		$outputlangs->loadLangs(array("main", "bills", "propal", "companies", "priceindexation@priceindexation"));
